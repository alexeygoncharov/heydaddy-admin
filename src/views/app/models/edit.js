...
export default class UpdateService extends Component {
    ...
    getSingleData = async ()=> {
        ...
        let response = await ApiCall.get(`${Url.EDIT_MODEL}/${this.props.match.params.id}`, await config());
        if(response.status === 200){
            this.setState({
                name: response.data.user.name,
                email: response.data.user.email,
                id: response.data.user._id,
                pricePerDay: response.data.user.pricePerDay,
                age: response.data.user.age,
                nationalUrl: response.data.user.nationalId?.path,
                profileUrl: response.data.user.profile?.path,
                selectedServices: response.data.user.services.map(item => {return { label: item.name, key: item._id, value: item._id }}),
                selectedLanguages: response.data.user.languages.map(item => {return { label: item.name, key: item._id, value: item._id }}),
                selectedCity: response.data.user.city,
                selectedCountry: response.data.user.country,
                selectedProvince: response.data.user.state,
                spinning: false
            });
        }
    }
                ...
                this.state.designation === ""? designationValidation :
                    this.state.pricePerDay === ""? priceperdayValidation :
                        this.state.age === ""? ageValidation :
                            this.state.profileStatus === "removed"? imageValidation :
                                this.state.nationalStatus === "removed"? nationalValidation :
                                    passed :
            nameValidation
    };
    ...
    updateModel = async (e)=> {
        ...
        const {name, age, email, password, pricePerDay, image,
            selectedCity,profileStatus,
            selectedCountry,nationalStatus,
        ...
        if(validation.status){
            ...
            data.append('age', age);
            data.append('nationalStatus', nationalStatus);
            data.append('profileStatus', profileStatus);
            data.append('profile', image);
            data.append('nationalId', national);
            let response = await ApiCall.post(`${Url.MODEL_UPDATE}/${this.props.match.params.id}`, data, await multipartConfig());
            if(response.status === 200){
                        ...
                        {spinning? <div className="loading" /> :
                            <Card>
                                ...
                                <CardBody>
                                    ...
                                    <Form className="dashboard-quick-post" onSubmit={this.updateModel}>
                                        <FormGroup row>
                                            <Label sm="3">
