...
export default class Posts extends Component {
    ...
    getAllLanguages = async ()=> {
        ...
        let response = await ApiCall.get(`${Url.GET_ALL_SERVICES}?page=${this.state.page}&limit=${this.state.displayLength}&q=${this.state.search}`, await config())
    ...
    shownDataLength = async(dataKey) =>{
        let response = await ApiCall.get(`${Url.GET_ALL_SERVICES}?page=${1}&limit=${dataKey}&q=${this.state.search}`, await config())
        if(response.status=== 200){
            this.setState({
                data: response.data.services,
                length: response.data.length
                ...
                <Row>
                    <Colxx xxs="12">
                        <div className="text-zero top-right-button-container">
                            <Link to='/app/service/create'><Button size='lg' color={'secondary'}><IntlMessages id={"menu.create"} /></Button></Link>
                        </div>
