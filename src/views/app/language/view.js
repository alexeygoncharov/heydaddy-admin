...
export default class Posts extends Component {
    ...
    getAllLanguages = async ()=> {
        ...
        let response = await ApiCall.get(`${Url.GET_ALL_LANGUAGES}?page=${this.state.page}&limit=${this.state.displayLength}&q=${this.state.search}`, await config())
    ...
    getAllLanguagesSearch = async (search)=> {
        let response = await ApiCall.get(`${Url.GET_ALL_LANGUAGES}?page=${1}&limit=${this.state.displayLength}&q=${search}`, await config())
        if(response.status=== 200){
            this.setState({
                data: response.data.languages,
                length: response.data.length});
    ...
    dataShownPagination = async(dataKey) => {
        let response = await ApiCall.get(`${Url.GET_ALL_LANGUAGES}?page=${dataKey}&limit=${this.state.displayLength}&q=${this.state.search}`, await config())
    ...
    shownDataLength = async(dataKey) =>{
        let response = await ApiCall.get(`${Url.GET_ALL_LANGUAGES}?page=${1}&limit=${dataKey}&q=${this.state.search}`, await config())
        if(response.status=== 200){
            this.setState({
                data: response.data.languages,
                length: response.data.length
    ...
    confirmDelete = async (item) => {
        this.setState({spinning: true});
        let response = await ApiCall.delete(`${Url.DELETE_LANGUAGE}/${item}`, await config());
        if(response.status === 200){
            this.setState({spinning: false});
            this.getAllLanguages();
                ...
                <Row>
                    <Colxx xxs="12">
                        <div className="text-zero top-right-button-container">
                            <Link to='/app/language/create'><Button size='lg' color={'secondary'}><IntlMessages id={"menu.create"} /></Button></Link>
                        </div>
                                                ...
                                                {(rowData, rowIndex) => {
                                                    return <div>
                                                        ...
                                                        <Button color="secondary" size="xs" className="mb-2">
                                                            <Link to={`/app/language/edit/${rowData._id}`} style={{color: 'white'}}><IntlMessages id="edit" /></Link>
                                                        </Button>}
                                                        {" "}{" "}
