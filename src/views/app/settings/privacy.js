...
import {Button, Card, CardBody, CardTitle, Col, Form, FormGroup, Row} from "reactstrap";
...
export default class PrivacyPolicy extends Component {
    ...
    updatePrivacyPolicy = async (e) => {
        ...
        let response = await ApiCall.post(Url.UPDATE_PRIVACY_POLICY, {
            policy: content,
        }, await config());
        if (response.status === 200) {
            this.getPrivacyPolicy();
    ...
    storeTranslation = async () => {
        ...
        if(validation.status){
            ...
            let response = await ApiCall.post(Url.STORE_PRIVACY_POLICY_TRANSLATIONS, {
    ...
    updateTranslation = async () => {
        ...
        if(validation.status){
            this.setState({loadingApi: true})
            let response = await ApiCall.post(Url.UPDATE_PRIVACY_POLICY_TRANSLATIONS, {
                language_id: selectedLanguage,
                entity_id:  entityId,
                policy: translationContent,
                id: translationId
    ...
    render() {
        ...
        return (
            <Fragment>
                <Row>
                    <Colxx xxs="12">
                        <Breadcrumb heading="menu.privacy-policy" match={this.props.match}/>
